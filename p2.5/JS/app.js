...
document
  ...
  .addEventListener("click", () => movieList.sortAZ());
document
  .querySelector("#sortZA")
  .addEventListener("click", () => movieList.sortZA());
document
  .querySelector("#sortRateHigh")
  .addEventListener("click", () => movieList.sortRatingHighLow());
...
document.getElementById("searchTitleForm").addEventListener("submit", (e) => {
  e.preventDefault();

  const value = document.getElementById("searchByTitle").value.trim();

  if (value === "") {
    showAlert("Please enter a Movie Title", "warning");
    return;
  }

  const count = movieList.searchByTitle(value);
